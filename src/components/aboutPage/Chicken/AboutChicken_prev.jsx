...
import svgStar from '../../../assets/aboutPage/aboutStar.svg'
...
const ChickenModel = ({ chickenModel, setModelLoaded }) => {
    ...
    const { iorR, iorY, iorG, iorC, iorB, iorP, saturation, chromaticAberration, refraction } = useControls({
        ior: folder({
            iorR: { min: 1.0, max: 2.333, step: 0.001, value: 1.08 },
            iorY: { min: 1.0, max: 2.333, step: 0.001, value: 1.08 },
            iorG: { min: 1.0, max: 2.333, step: 0.001, value: 1.1 },
            iorC: { min: 1.0, max: 2.333, step: 0.001, value: 1.12 },
            iorB: { min: 1.0, max: 2.333, step: 0.001, value: 1.09 },
            iorP: { min: 1.0, max: 2.333, step: 0.001, value: 1.1 },
        }),
    ...
    useFrame((state) => {
        ...
        // backgroundColor.current.visible = true
        ...
        // backgroundColor.current.visible = false
    ...
    // const columns = range(-60, 20, 25)
    // const rows = range(-30, 40, 20)
    ...
    return (
        <>
            {/* <color ref={backgroundColor} attach="background" args={['#ff7777']} /> */}
            <group ref={backgroundGroup}>
                ...
                {columns.map((col, i) =>
                    rows.map((row, j) => (
                        // <mesh key={i + j + 1} position={[col, row, -120]}>
                        //     <Svg src={svgStar} />
                        //     <meshBasicMaterial color="white" />
                        // </mesh>
    ...
    return (
        <div className="aboutChicken__wrapper">
            <div className="aboutChicken__3DModel">
                <Canvas dpr={[1, 2]} camera={{ position: [0, 0, 16.8], fov: 45 }}>
